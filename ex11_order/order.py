...
class OrderAggregator:
    ...
    def aggregate_order(self, customer: str, max_items_quantity: int, max_volume: int):
        ...
        for item in self.order_items:
            if customer == item.customer and item.quantity <= max_items_quantity and item.total_volume <= max_volume:
                items.append(item)
                max_volume -= item.one_item_volume
                max_items_quantity -= 1
        for item in items:
...
class ContainerAggregator:
    ...
    def prepare_containers(self, orders: tuple) -> dict:
        ...
        for order in orders:
            ...
            kontejner = Container(self.container_volume, [order])
            if self.container_volume >= order.total_volume:
                if dest in containers:
                    for elem in range(len(containers[dest])):
                        if order.total_volume <= containers[dest][elem].volume_left:
                            containers[dest][elem].orders.append(order)
                            break
                        if order.total_volume > containers[dest][elem].volume_left and elem == (len(containers[dest]) - 1):
                            containers[dest].append(kontejner)
                            break
                        if order.total_volume > containers[dest][elem].volume_left and elem < len(containers[dest]) - 1:
                            continue
                else:
