...
def get_competitors_list(filename: str) -> list:
    ...
    """
    with open(filename) as competition:
        return competition.read().splitlines()
...
def get_results_dict(filename: str) -> dict:
    ...
    """
    competition = open(filename)
    dict = {}
    for i in competition:
        (name, result) = i.split(" - ")
        dict[name] = int(result.rstrip("\n"))
    return dict
...
def filter_results(path_to_competitors: str, path_to_results: str) -> dict:
    ...
    result = get_results_dict(path_to_results)
    subdict = {x: result[x] for x in comp if x in result}
    return subdict
...
def sort_results(path_to_competitors: str, path_to_results: str) -> list:
    ...
    """
    res = filter_results(path_to_competitors, path_to_results)
    b = dict(sorted(res.items(), key=lambda value: value[1], reverse=True))
    list_result = []
...
def write_results_csv(path_to_competitors: str, path_to_results: str, file_to_write: str) -> None:
    ...
    :return: None
    """
    sort = sort_results(path_to_competitors, path_to_results)
    with open(file_to_write, "w", newline="") as file:
        writer = csv.writer(file, delimiter=",")
        writer.writerow(["Place", "Name", "Result"])
        for inn in sort:
            writer.writerow([sort.index(inn) + 1, inn[0], inn[1]])
